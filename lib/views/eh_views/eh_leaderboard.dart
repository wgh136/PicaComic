...
class EhLeaderboardLogic extends StateController {
  ...
  var networkStatus = <bool>[false, false, false, false];
}
...
class OneEhLeaderboardPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StateBuilder<EhLeaderboardLogic>(builder: (logic) {
      if (logic.leaderboards[index].galleries.isEmpty &&
          !logic.networkStatus[index]) {
        EhNetwork()
            .getLeaderboard(logic.leaderboards[index].type)
            .then((board) {
          if (board.success) {
        ...
        slivers: [
          SliverGrid(
            delegate: SliverChildBuilderDelegate(
                ...
                (context, i) {
              if (i == logic.leaderboards[index].galleries.length - 1 &&
                  logic.leaderboards[index].loaded != EhLeaderboard.max) {
                EhNetwork()
                    .getLeaderboardNextPage(logic.leaderboards[index])
                    .then((v) => logic.update());
              }
          ...
          if (logic.leaderboards[index].loaded != EhLeaderboard.max &&
              !logic.networkStatus[index])
