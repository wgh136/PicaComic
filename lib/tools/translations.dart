...
extension AppTranslation on String {
  ...
  static const Map<String, Map<String, String>> translations = {
    ...
    "zh_TW": {
      ...
      "导出不含下载的数据": "導出不含下載的數據",
      "导出所有数据": "導出所有數據",
      "如需还原之前的下载, 将路径填写为下载数据的位置, 并取消勾选转移数据": "如需還原之前的下載, 將路徑填寫為下載數據的位置, 並取消勾選轉移數據",
      "如果导入的数据中包含下载数据, 则当前的下载数据也将被覆盖": "如果導入的數據中包含下載數據, 則當前的下載數據也將被覆蓋",
      "缓存大小限制": "緩存大小限制",
      "缓存数量限制": "緩存數量限制",
      "仅在退出阅读器时检查缓存是否超出限制": "僅在退出閱讀器時檢查緩存是否超出限制",
      "需要身份验证": "需要身份驗證",
      "添加漫画源": "添加漫畫源",
      "选择文件": "選擇文件",
      "优先显示副标题": "優先顯示副標題",
...
""",
      ...
      "如需还原之前的下载, 将路径填写为下载数据的位置, 并取消勾选转移数据":
          "To restore previous downloads, fill in the path as the location of the downloaded data, and uncheck the \"Transfer data\".",
      "如果导入的数据中包含下载数据, 则当前的下载数据也将被覆盖":
          "If the imported data contains download data, the current download data will also be overwritten",
